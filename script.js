document.addEventListener('DOMContentLoaded', function() {
  ...
  // Helper function to check if a group contains a "Situation" question
  function isSituationGroup(group) {
    return group.some(question => 
      question.stem && question.stem.toLowerCase().includes('situation')
    );
  }
  
  // Helper function to calculate number of situation groups based on your rule
  function calculateSituationGroups(totalQuestions) {
    const calculation = (totalQuestions - 5) / 3;
    // Round down to nearest multiple of 5
    return Math.floor(calculation / 5) * 5;
  }
  
  // Main randomization function with Situation group placement rule
  function randomizeExam(groups, totalQuestions = 20) {
    // Convert groups object to array of groups
    const groupArray = Object.values(groups);
    
    // Separate situation groups from other groups
    const situationGroups = groupArray.filter(group => isSituationGroup(group));
    const otherGroups = groupArray.filter(group => !isSituationGroup(group));
    
    // Calculate the number of situation groups to include based on the rule
    const numSituationGroups = calculateSituationGroups(totalQuestions);
    
    // Randomize both groups
    const randomizedSituationGroups = shuffleArray(situationGroups);
    const randomizedOtherGroups = shuffleArray(otherGroups);
    
    // Select the required number of situation groups
    const selectedSituationGroups = randomizedSituationGroups.slice(0, numSituationGroups);
    
    // Create the final randomized exam
    const randomizedExam = [];
    
    // Add situation groups first (with placement rule)
    let remainingPositions = totalQuestions;
    
    selectedSituationGroups.forEach(group => {
      const groupSize = group.length;
      
      // Calculate the last valid position for this situation group
      const maxPosition = remainingPositions - groupSize + 1;
      
      // If there's no valid position, skip this group
      if (maxPosition <= 0) return;
      
      // Place the group at a random valid position
      randomizedExam.push(...group);
      remainingPositions -= groupSize;
    });
    
    // Fill remaining positions with other groups
    randomizedOtherGroups.forEach(group => {
      if (remainingPositions <= 0) return;
      
      const groupSize = group.length;
      if (groupSize > remainingPositions) {
        // If group is too big, take first few questions to fill remaining space
        randomizedExam.push(...group.slice(0, remainingPositions));
    ...
    // If there are still remaining positions, add single questions from remaining groups
    if (remainingPositions > 0) {
      ...
      randomizedOtherGroups.forEach(group => {
        group.forEach(question => {
          if (!randomizedExam.some(q => q.id === question.id)) {
            allRemainingQuestions.push(question);
          }
        });
      });
      
      // Shuffle and take the first few needed questions
      const additionalQuestions = shuffleArray(allRemainingQuestions).slice(0, remainingPositions);
      randomizedExam.push(...additionalQuestions);
    }
    
    // Ensure we have exactly totalQuestions
    if (randomizedExam.length > totalQuestions) {
      return randomizedExam.slice(0, totalQuestions);
    }
  ...
  // ANSWER DISTRIBUTION
  // ======================
  function computeAnswerDistribution(exam) {
    const counts = {
      A: 0,
      B: 0,
      C: 0,
    ...
    return {
      A: Math.round((counts.A / counts.total) * 100) || 0,
      B: Math.round((counts.B / counts.total) * 100) || 0,
      C: Math.round((counts.C / counts.total) * 100) || 0,
      D: Math.round((counts.D / counts.total) * 100) || 0,
      total: counts.total
    };
  }
  ...
  document.addEventListener('click', function(e) {
    if (e.target.id === 'close-image-modal') {
